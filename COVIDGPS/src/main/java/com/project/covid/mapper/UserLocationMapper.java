...
public interface UserLocationMapper {
	...
	@Select("SELECT mcode,loc,inTime,outTime from UserLocation WHERE id=#{id}")
	...
	@Insert("Insert into UserLocation values(#{id},#{mcode},#{loc},#{inTime},#{outTime})")
	void putUserIdLocation(@Param("id")int id,@Param("mcode")String mcode,@Param("loc")String loc,@Param("inTime")Date inTime,@Param("outTime")Date outTime);
	
	@Select("SELECT loc from OwnerProfile where hostID=#{hostID}")
	String gethostIdLocation(@Param("hostID")Integer hostID);
	
	@Select("SELECT id from UserLocation WHERE loc=#{loc} and (( inTime <= #{inTime} and #{inTime} <= outTime) or (inTime<=#{outTime} and outTime<=#{outTime}))")
